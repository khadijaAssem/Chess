...
void check_Content(moves X){
    unsigned char Des = maze[X.DesR][X.DesC];
    printf("---%%%c\n",Des);
    //unsigned char Cr = maze[X.CrR][X.CrC];
    if ((Des!='.')&&(Des!='-')){
        if(Des >= 'A' && Des <= 'Z'){//Upper case , to Player one
            if (X.ID == 1){
                Out02[N02]=Des;
                N01++;
                return;
        ...
        else if(Des >= 'a' && Des <= 'z'){//I can remove the condition
            if (X.ID == 2){
                Out01[N01]=Des;
                N01++;
                return;
